...
from .base_v0 import BaseV0
from ..env_base import get_sim
from ...utils.quatmath import euler2quat
from ...utils.vectormath import calculate_cosine
...
class BaodingFixedEnvV1(BaseV0):
    ...
    def _setup(
        ...
        **kwargs
        ...
        self.center_pos = [-0.0125, -0.05]  # [-.0020, -.0522]
        self.counter = 0
    ...
    def step(self, a):
        ...
        self.sim_obsd.model.site_pos[self.target1_sid, 0] = desired_positions_wrt_palm[
            0
        ]
        self.sim_obsd.model.site_pos[self.target1_sid, 1] = desired_positions_wrt_palm[
            1
        ]
        self.sim_obsd.model.site_pos[self.target2_sid, 0] = desired_positions_wrt_palm[
            2
        ]
        self.sim_obsd.model.site_pos[self.target2_sid, 1] = desired_positions_wrt_palm[
            3
        ]
    ...
    def get_obs_vec(self):
        ...
        self.obs_dict["target1_err"] = (
            self.obs_dict["target1_pos"] - self.obs_dict["object1_pos"]
        )
        self.obs_dict["target2_err"] = (
            self.obs_dict["target2_pos"] - self.obs_dict["object2_pos"]
        )
    ...
    def get_reward_dict(self, obs_dict):
        ...
        hand_pos = (
            obs_dict["hand_pos"][:, :, :3]
            if obs_dict["hand_pos"].ndim == 3
            else obs_dict["hand_pos"][:3]
        )
        ...
        rwd_dict = collections.OrderedDict(
            (
                ...
                (
                    "bonus",
                    (target1_dist < self.proximity_threshold)
                    + (target2_dist < self.proximity_threshold)
                    + 4
                    * (target1_dist < self.proximity_threshold)
                    * (target2_dist < self.proximity_threshold),
                ),
    ...
    def reset_model(self, reset_pose=None, reset_vel=None, reset_goal=None):
        ...
        # reset goal
        self.goal = (
            self.create_goal_trajectory() if reset_goal is None else reset_goal.copy()
        )
    ...
    def get_env_state(self):
        ...
        mocap_pos = (
            self.sim.data.mocap_pos.copy() if self.sim.model.nmocap > 0 else None
        )
        mocap_quat = (
            self.sim.data.mocap_quat.copy() if self.sim.model.nmocap > 0 else None
        )
        site_pos = (
            self.sim.model.site_pos[:].copy() if self.sim.model.nsite > 0 else None
        )
    ...
    def evaluate_success(self, paths, logger=None):
        ...
        for path in paths:
            num_success += (
                np.sum(path["env_infos"]["solved"][-5:], dtype=np.int)
        ...
        if logger:
            ...
            rwd_dense = np.mean(
                [np.sum(p["env_infos"]["rwd_dense"]) / horizon for p in paths]
            )  # return rwd/step
